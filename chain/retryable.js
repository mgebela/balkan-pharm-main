...
export function isRetryableChainError(err) {
  ...
  if (/Payment transaction failed on-chain|Payment too low|Payment does not debit|Payment signature already used|Invalid growth stage|Journal proof failed|missing buyerPubkey|missing paymentSignature/i.test(msg)) {
    return false;
  }
