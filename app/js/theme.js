 ...
 *
 * Dark is the default AND the unthemed state: only `light` is ever written
 * to <html data-theme>. That is what lets the first paint be correct before
 * any script runs, and it is why there is no prefers-color-scheme rule in
 * the stylesheets — the OS only gets a say once the grower picks Auto.
 *
...
(function () {
  ...
  function read() {
    try {
      ...
      return VALID.indexOf(v) === -1 ? 'dark' : v;
    } catch (e) {
