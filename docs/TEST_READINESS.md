...
| **Plant camera** | Full in-app camera (preview, shutter, Flip, Gallery); **Log to journal** / **Ask coach** / Retake. Save errors use the paper sheet, not `alert` | Coach control uses a **camera** icon (not `+`); Netlify allows `camera=(self)` |
| **Stories** | Delete story uses the same paper confirm sheet as Market / Tokenise | **Untested.** Confirm the browser `confirm()` is gone |
